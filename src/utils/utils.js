...
const defaults = {
  ...
  BOT_CHANNEL_ID: `687263659885330486`,
  NOTIF_CHANNEL_ID: `688684846419148811`,
  DEBUG_USER_ID: `105899837517475840`,
};
...
function printReply(msg, replyObj) {
  ...
  switch (replyObj.status) {
    ...
    case 'full':
      return msg.reply(`Sorry. Your list is full. ${replyObj.count}/${defaults.MAX_LIST_SIZE}`);
    case 'empty':
      return msg.reply(`Your list is empty.`);
    case 'notOnList':
      return msg.reply(`Sorry. #${replyObj.itemID} is not on your current list.`);
    case 'clear':
      return msg.reply(`List cleared.`);
    case 'notFound':
      return msg.reply(`#${replyObj.itemID} not found.`);
  }
...
function printArrAsTable(arr, itemType, equipLocation) {
  if (arr.length < 1) return 'printArrAsTable: Cannot print empty array.';
  if (arr.length > 22)
  ...
  let isEquip = true;
  if (itemType === 'Card' || equipLocation === '-') isEquip = false;
  let titleObj;
